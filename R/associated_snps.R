...
associated_snps <- function(genes=c(),
  ...
  df <- data.frame("Genes"=genes, "Associated_snps"="", "Traits_associated_to_snps"="", 
  ...
  lapply(snps, function(g){
    ...
    call1 = paste0('https://www.ebi.ac.uk/gwas/rest/api/singleNucleotidePolymorphisms/search/findByGene?',
                   g)
    ...
    if(get_assoc_text != "" & ! grepl("Internal Server Error", get_assoc_text)){
      ...
      df$Traits_associated_to_snps[df$Genes == g] <<- 
        paste0(unique(unlist(strsplit(temp$Associated_traits, split="; "))), collapse=delim)
